...
import { Fish, Trophy, Info, Sparkles, BrainCircuit, MessageSquare, RotateCcw, RotateCw, Link as LinkIcon, X, BookOpen, Copy, Users, Monitor, Smartphone } from 'lucide-react';
...
const appId = 'xok-webapp'; // Pots posar el nom que vulguis
...
export default function XokGameHex() {
  ...
  const [playerColor, setPlayerColor] = useState(null); // 'white' o 'black'
  const [isJoined, setIsJoined] = useState(false);
  const [isLocal, setIsLocal] = useState(false); // Mode local vs Online
  ...
  const startLocalGame = () => {
    setRoomId(null);
    setPlayerColor(null); // Tots dos juguen
    setIsLocal(true);
    setIsJoined(true);
  ...
  const exitLobby = () => {
    ...
    setIsLocal(false);
    setWinner(null);
    // Refresh page equivalent to clear states cleanly
    setBoard(generateBoardCells());
    setSupply(JSON.parse(JSON.stringify(INITIAL_SUPPLY)));
  };

  const updateGameState = async (newBoard, newSupply, nextTurn, newLogs, newWinner = null, newReason = '') => {
    // Si és local, actualitza estat React directament
    if (isLocal) {
        setBoard(newBoard);
        setSupply(newSupply);
        setTurn(nextTurn);
  ...
  const impactedCells = useMemo(() => (hoverCell && selectedAction === 'shark') ? getImpactedCells(hoverCell.q, hoverCell.r) : [], [hoverCell, selectedAction, currentMouths, board, turn]);
  ...
  const endTurnDB = async (newBoard, newSupply) => {
    ...
    setPhase('SELECT_ACTION'); setSelectedAction(null); setTempMove({});
  ...
  const handleCellClick = (cell) => {
    ...
    // En local qualsevol pot moure si és el seu torn. En online només el propietari.
    ...
    if (selectedAction === 'fish') {
      ...
      if (phase === 'PLACING_FISH_1') {
        ...
        const isAdj = neighbors.some(n => n.q === q && n.r === r);
        if (!isAdj) return; 
    ...
    if (selectedAction === 'shark') {
      if (cell.owner === turn || (cell.type && cell.type.includes('shark'))) return;
      ...
      if (fishToEatIndices.length === 0) return; 
  ...
  const SharkConfigPanel = () => {
    ...
    const btnCls = (type) => `flex-1 py-2 text-[10px] font-bold rounded-lg border flex flex-col items-center justify-center gap-1 transition-all ${currentType === type ? 'bg-white border-teal-500 text-teal-700 shadow-sm ring-1 ring-teal-200' : 'bg-slate-100 border-transparent text-slate-400 hover:bg-slate-200'}`;
    return (
        <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 animate-in slide-in-from-left-4">
            ...
            <div className="grid grid-cols-4 gap-1 mb-4">
                <button onClick={() => setSharkSelection({type: PIECE_TYPES.SHARK_SMALL, rotation: 0})} className={btnCls(PIECE_TYPES.SHARK_SMALL)} disabled={supply[turn].shark_small === 0}><span>S (1)</span></button>
                <button onClick={() => setSharkSelection({type: PIECE_TYPES.SHARK_BIG_60, rotation: 0})} className={btnCls(PIECE_TYPES.SHARK_BIG_60)} disabled={supply[turn].shark_big_60 === 0}><span>60°</span></button>
                <button onClick={() => setSharkSelection({type: PIECE_TYPES.SHARK_BIG_120, rotation: 0})} className={btnCls(PIECE_TYPES.SHARK_BIG_120)} disabled={supply[turn].shark_big_120 === 0}><span>120°</span></button>
                <button onClick={() => setSharkSelection({type: PIECE_TYPES.SHARK_BIG_180, rotation: 0})} className={btnCls(PIECE_TYPES.SHARK_BIG_180)} disabled={supply[turn].shark_big_180 === 0}><span>180°</span></button>
            </div>
            <div className="flex items-center justify-between mb-4">
                <button onClick={() => rotate('ccw')} className="p-2 bg-white rounded-full border border-slate-200 hover:bg-slate-50 text-slate-500"><RotateCcw size={16}/></button>
                ...
                <button onClick={() => rotate('cw')} className="p-2 bg-white rounded-full border border-slate-200 hover:bg-slate-50 text-slate-500"><RotateCw size={16}/></button>
            </div>
  ...
  const renderCell = (cell) => {
    ...
    const isHovered = hoverCell && hoverCell.q === cell.q && hoverCell.r === cell.r;
    let isHighlight = false, isValidTarget = false, isImpacted = false, showGhostShark = false;

    // Només calcular visuals si és el teu torn (o mode local)
    if (isLocal || turn === playerColor) {
        if (selectedAction === 'fish') {
           ...
           if (phase === 'PLACING_FISH_2') {
             ...
             if (!cell.type) { const ns = getNeighbors(tempMove.q1, tempMove.r1); if (ns.some(n => n.q === cell.q && n.r === cell.r)) isValidTarget = true; }
           }
        }
        if (selectedAction === 'shark') {
           const canPlaceShark = !cell.owner || (cell.owner !== turn && (!cell.type || !cell.type.includes('shark')));
           if (canPlaceShark) isValidTarget = true;
           if (isHovered && canPlaceShark) showGhostShark = true;
    ...
    return (
      <div key={`${cell.q},${cell.r}`} onClick={() => handleCellClick(cell)} onMouseEnter={() => setHoverCell(cell)} onMouseLeave={() => setHoverCell(null)}
        style={{ position: 'absolute', left: `calc(50% + ${x + centerX}px)`, top: `calc(50% + ${y + centerY}px)`, width: `${HEX_WIDTH}px`, height: `${HEX_HEIGHT}px`, marginLeft: `-${HEX_WIDTH/2}px`, marginTop: `-${HEX_HEIGHT/2}px`, clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)", zIndex: 10, cursor: (isLocal || turn === playerColor) ? 'pointer' : 'default' }}
        className={`flex items-center justify-center transition-all duration-200 ${isImpacted ? 'bg-rose-500/80 animate-pulse border-rose-600 border-2' : ''} ${!isImpacted && isValidTarget ? 'bg-teal-400 hover:bg-teal-300' : ''} ${!isImpacted && !isValidTarget ? 'bg-white/20 hover:bg-white/30' : ''} ${isHighlight ? 'bg-teal-500' : ''} ${!isValidTarget && !isHighlight && !isImpacted ? 'backdrop-blur-[1px]' : ''}`}>
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full animate-in fade-in zoom-in duration-300 relative">
          ...
          <div className="space-y-3">
            <Button onClick={startLocalGame} className="w-full py-4 text-lg shadow-teal-500/30 bg-indigo-600 hover:bg-indigo-700 gap-3">
                <Monitor size={20}/> {t('lobby_local')}
            </Button>
            <div className="relative py-2">
                <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-200"></div></div>
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-slate-400">{t('lobby_online_divider')}</span></div>
            </div>
            <Button onClick={createRoom} className="w-full py-3 gap-3" variant="secondary">
                <Users size={20}/> {t('lobby_create')}
            </Button>
            <div className="flex gap-2">
                <input 
                type="text" 
                placeholder={t('lobby_id_ph')} 
                className="flex-1 px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 outline-none font-mono uppercase text-center"
                value={inputRoomId}
                onChange={(e) => setInputRoomId(e.target.value)}
                />
                <Button onClick={joinRoom} variant="outline">{t('lobby_enter')}</Button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-100 font-sans text-slate-800 flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="w-full md:w-80 bg-white shadow-xl flex flex-col z-20 border-r border-slate-200">
        <div className="p-6 border-b border-slate-100 bg-white flex flex-col gap-4">
           <div className="flex items-center justify-between">
             <div className="flex items-center gap-3">
               ...
               <div>
                   <h1 className="text-2xl font-black text-slate-900 leading-none">XOK</h1>
                   <span className="text-[10px] font-bold text-slate-400 tracking-wider">
                       {isLocal ? t('local_mode_badge') : t('online_mode_badge')}
                   </span>
               </div>
             </div>
             <button onClick={exitLobby} className="text-xs font-bold text-slate-400 hover:text-red-500 flex flex-col items-center">
                 <X size={16}/> {t('exit_lobby')}
             </button>
           </div>
           
           {/* ROOM ID WIDGET (Only Online) */}
           {!isLocal && (
               <div className="bg-slate-100 p-3 rounded-xl flex items-center justify-between border border-slate-200">
                  <div className="flex items-center gap-2 text-xs text-slate-500 font-bold">
                     <Users size={14}/> ID: <span className="font-mono text-slate-800 text-sm select-all">{roomId}</span>
                  </div>
                  <button onClick={() => {navigator.clipboard.writeText(roomId); alert("ID Copiat!")}} className="p-1 hover:bg-white rounded"><Copy size={14} className="text-slate-400"/></button>
               </div>
           )}
           
           <div className={`text-xs font-bold text-center py-1 px-2 rounded-lg ${turn === playerColor || isLocal ? 'bg-green-100 text-green-700 animate-pulse' : 'bg-slate-100 text-slate-400'}`}>
              {isLocal 
                ? `${t('log_turn')} ${turn === PLAYERS.WHITE ? t('white') : t('black')}` 
                : (turn === playerColor ? "ÉS EL TEU TORN!" : "ESPERANT RIVAL...")
              }
           </div>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col gap-4 overflow-y-auto">
          {/* Supply & Score */}
          <div className="grid grid-cols-2 gap-3 mb-4 bg-slate-50 p-3 rounded-2xl border border-slate-200">
             <div className={`text-center p-2 rounded-xl transition-all ${turn === PLAYERS.WHITE ? 'bg-white shadow-md ring-2 ring-teal-500' : 'opacity-50 grayscale'}`}>
                ...
                <div className="flex flex-col gap-1 text-xs mb-2">
                   <div className="flex justify-between items-center"><Fish size={14}/> <b>{supply.white.fish}</b></div>
                   <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-slate-500">S</span> <b>{supply.white.shark_small}</b></div>
                   <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-slate-500">L</span> <b>{supply.white.shark_big_60+supply.white.shark_big_120+supply.white.shark_big_180}</b></div>
                </div>
                <div className={`mt-1 pt-1 border-t border-slate-100 flex justify-between items-center ${chainLengths.white >= 10 ? 'text-green-600 font-bold' : 'text-slate-600'}`}><span className="text-[10px] uppercase font-bold tracking-tighter">{t('chain')}</span><span className="flex items-center gap-1"><LinkIcon size={12}/> {chainLengths.white}</span></div>
             </div>
             <div className={`text-center p-2 rounded-xl transition-all ${turn === PLAYERS.BLACK ? 'bg-slate-900 text-white shadow-md ring-2 ring-teal-500' : 'opacity-50 grayscale'}`}>
                ...
                <div className="flex flex-col gap-1 text-xs mb-2">
                   <div className="flex justify-between items-center"><Fish size={14}/> <b>{supply.black.fish}</b></div>
                   <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-slate-400">S</span> <b>{supply.black.shark_small}</b></div>
                   <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-slate-400">L</span> <b>{supply.black.shark_big_60+supply.black.shark_big_120+supply.black.shark_big_180}</b></div>
                </div>
                <div className={`mt-1 pt-1 border-t border-slate-700 flex justify-between items-center ${chainLengths.black >= 10 ? 'text-green-400 font-bold' : 'text-slate-300'}`}><span className="text-[10px] uppercase font-bold tracking-tighter">{t('chain')}</span><span className="flex items-center gap-1"><LinkIcon size={12}/> {chainLengths.black}</span></div>
             </div>
          </div>
          ...
          {!winner && (isLocal || turn === playerColor) && (
            <div className="space-y-3 pt-4 border-t border-slate-100">
               ...
               <Button onClick={() => { if (supply[turn].fish >= 2) { setSelectedAction('fish'); setPhase('PLACING_FISH_1'); } }} disabled={supply[turn].fish < 2} className={`w-full justify-between transition-all ${selectedAction === 'fish' ? 'ring-2 ring-teal-500 bg-teal-50 border-teal-200 text-teal-800' : ''}`} variant={selectedAction === 'fish' ? 'ghost' : 'primary'}>
                 <div className="flex items-center gap-2"><Fish size={20}/> {t('fish_btn')}</div><div className="text-xs opacity-70">{t('fish_sub')}</div>
               </Button>
               <Button onClick={() => { if (supply[turn].shark_small > 0 || supply[turn].shark_big_60 > 0 || supply[turn].shark_big_120 > 0 || supply[turn].shark_big_180 > 0) { setSelectedAction('shark'); setPhase('PLACING_SHARK'); setTempMove({}); } }} disabled={supply[turn].shark_small === 0 && supply[turn].shark_big_60 === 0 && supply[turn].shark_big_120 === 0 && supply[turn].shark_big_180 === 0} className={`w-full justify-between transition-all ${selectedAction === 'shark' ? 'ring-2 ring-rose-500 bg-rose-50 border-rose-200 text-rose-800' : ''}`} variant={selectedAction === 'shark' ? 'ghost' : 'primary'}>
                 <div className="flex items-center gap-2"><SharkIcon size={20} /> {t('shark_btn')}</div><div className="text-xs opacity-70">{t('shark_sub')}</div>
               </Button>
               {selectedAction === 'shark' && <SharkConfigPanel />}
      ...
      {/* GAME BOARD AREA */}
      <div className="flex-1 bg-cyan-900 overflow-hidden relative">
        <MarinePattern />
        <div className="absolute inset-0 flex items-center justify-center"><div className="relative w-[800px] h-[800px]">{board.map(cell => renderCell(cell))}</div></div>
      </div>
      ...
      {winner && (
        <Modal title={t('game_over')} onClose={exitLobby}>
          <div className="text-center py-4">
             ...
             <div className="bg-teal-50 text-teal-800 px-4 py-2 rounded-lg font-bold">{winReason}</div>
             <Button onClick={exitLobby} className="w-full mt-6">{t('exit_lobby')}</Button>
          </div>
        </Modal>
      )}
      {showRules && (
        <Modal title={t('rules_title')} onClose={() => setShowRules(false)}>
